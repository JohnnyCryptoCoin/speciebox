...
public class WalletController2 {
	...
	protected final HDWalletKit2 SPECIEBOX;
    ...
    public WalletController2(NetworkParameters params, String walletDirectory, int threshold){
    	// Determine what network params we are going to handle
    	this(params, walletDirectory, "specie-wallet_" + now(), threshold);
    }
    
    public WalletController2(NetworkParameters params, String walletDirectory, String fileName, int threshold){
    	this.params = params;
    	this.isEncrypted = false;
    	this.filePrefix = fileName;
		SPECIEBOX = new HDWalletKit2(params, new File(walletDirectory), fileName, threshold);
    }
    
    public WalletController2(NetworkParameters params, String walletDirectory, int threshold, List<DeterministicKey> followingKeys){
    	this.params = params;
    	this.isEncrypted = false;
...
//    	SPECIEBOX = new HDWalletKit(params, new File(walletDirectory), filePrefix, threshold, true, followingKeys);
    	SPECIEBOX = new HDWalletKit2(params, new File(walletDirectory), filePrefix, threshold, true, followingKeys);
    }
	...
	public void addFollowingWallet(String description, PluggableTransactionSigner signer){
//		SPECIEBOX.addPairedWallet(description, signer, false);
	}
	
	public void addMarriedWallet(String description, PluggableTransactionSigner signer){
